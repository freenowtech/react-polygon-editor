...
describe('helpers', () => {
    ...
    describe('isCoordinateInPolygon', () => {
        ...
        it('should emit true if point is inside the polygon', () => {
            expect(isCoordinateInPolygon(getCenterCoordinate(MOCK_POLYGON[0], MOCK_POLYGON[MOCK_POLYGON.length / 2]), MOCK_POLYGON)).toBeTruthy();
        });
    });
    ...
    describe('movePolygonCoordinates', () => {
        ...
        it('should move selected points', () => {
            ...
            expect(movedPolygon.every((coordinate, index) => index === 5 || coordinate === MOCK_POLYGON[index])).toBeTruthy();
        });
        ...
        describe('WHEN one closing point is selected', () => {
            it('should move both closing points', () => {
                ...
                expect(movedPolygon[MOCK_POLYGON.length - 1]).toEqual(addCoordinates(MOCK_POLYGON[MOCK_POLYGON.length - 1], MOVE_VECTOR));
            });
        });
    });
    ...
    describe('removeSelectedPoints', () => {
        ...
        it('should remove selected points ', () => {
            expect(removeSelectedPoints(SMALL_POYLGON, new Set<number>([1])))
        ...
        it('should open the polygon if less than 4 points are left', () => {
            expect(removeSelectedPoints(SMALL_POYLGON, new Set<number>([1, 2])))
                .toEqual([SMALL_POYLGON[3], SMALL_POYLGON[0]]);
        });

        it('should remove both edge points if one was selected', () => {
            expect(removeSelectedPoints(SMALL_POYLGON, new Set<number>([0, 1])))
